...
Value PieceValue[PHASE_NB][PIECE_NB] = {
  {PawnValueMg, KnightValueMg, BishopValueMg, RookValueMg, QueenValueMg, VALUE_ZERO, VALUE_ZERO, VALUE_ZERO  },
  {PawnValueEg, KnightValueEg, BishopValueEg, RookValueEg, QueenValueEg, VALUE_ZERO,  VALUE_ZERO, VALUE_ZERO }
};
...
void init() {
  ...
  {
      PieceValue[MG][8+pc] = PieceValue[MG][pc];
      PieceValue[EG][8+pc] = PieceValue[EG][pc];

      Score score = S(PieceValue[MG][pc], PieceValue[EG][pc]);

      for (Square s = SQ_A1; s <= SQ_H8; ++s)
      {
          File f = std::min(FileOf(s), ~FileOf(s));
          psq[pc][s] = score;// +Bonus[pc][RankOf(s)][f];
          psq[8+pc][~s] = psq[pc][s];
      }
  }
}
